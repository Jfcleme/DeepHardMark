...
def compute_predictions_labels(model, images, epsilon, G, args, imgnet_normalized_ops):
	...
	return predictions_labels.detach(), image_s.detach()  # .detach() is similar to .data in pytorch 0.3.1, that is to return a tensor
				...
				for j in range(w):
					...
					for k in range(c):
						if weight_type == 'variance':
							...
							weight[0, k, i, j] = (torch.max(image[0, k, up:down, left:right])-torch.min(image[0, k, up:down, left:right]))/(
									torch.max(image[0, k, up:down, left:right])+torch.min(image[0, k, up:down, left:right]))
						elif weight_type == 'contrast_mean':
							contrast = (torch.max(image[0, k, up:down, left:right])-torch.min(image[0, k, up:down, left:right]))/(
									torch.max(image[0, k, up:down, left:right])+torch.min(image[0, k, up:down, left:right]))
